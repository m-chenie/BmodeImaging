...
int main(){
    ...
    for(int i = 1; i < parameters->getNumElement(); i++){
    ...
    BmodeClass *currentScanline = scanlineHead; // creating BmodeClass pointer to scanlineHead
    dataBuffer *dataIterator = dataHead; // creating dataBuffer pointer to dataHead

    // iterating through linked list of dataHead and creating linked list from scanlineHead
    for (int i = 1; i < parameters->getNumScanline(); i++){ 
        dataIterator = dataIterator->next; // updating pointer to the next item in linked list
        currentScanline->next = new BmodeClass(parameters, dataIterator, i); // pointing currentScanline's next pointer to a new created BmodeClass object
        currentScanline = currentScanline->next; // updating currentScanline
        currentScanline->next = NULL; // setting currentScanline's next pointer to null
    }
...
/*
g++ ImageDisplay.cpp EBO.cpp VAO.cpp VBO.cpp Texture.cpp shaderClass.cpp glew.c -Llib -lglfw3 -lopengl32 -lwinmm -lgdi32 - Iinclude -DGLEW_STATIC Bmode.cpp dataBuffer.cpp BmodeClass.cpp imageParam.cpp -o bmode
*/